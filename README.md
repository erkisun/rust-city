# 🏙️ Rust City Chronicles

![Rust City Banner](images/banner.png)

**Lerne Rust durch Geschichten und Abenteuer!**  
Begleite Detective Ownership, Officer Borrowing und andere Charaktere in der sichersten Programmierstadt aller Zeiten.

## 📚 Kapitel

| Kapitel | Titel | Charakter | Konzept | Lesen |
|---------|-------|-----------|---------|-------|
| 1 | [Willkommen in Rust City](chapters/01-welcome-to-rust-city.md) | Bürgermeister `fn main()` | Grundstruktur | ✅ |
| 2 | [Detective Ownership und der verschwundene String](chapters/02-detective-ownership.md) | Detective Ownership | Ownership, Move | 🚧 |
| 3 | *Coming Soon* | Officer Borrowing | References, Borrowing | ⏳ |

## 👥 Die Charaktere von Rust City

- [Detective Ownership](characters/detective-ownership.md) 👮
- [Officer Borrowing](characters/officer-borrowing.md) 🚔  
- [Bürgermeister `fn main()`](characters/buergermeister-main.md) 🎩
- [Mutation Max](characters/mutation-max.md) ✍️
- [Shadow Sam](characters/shadow-sam.md) 🎭

## 🚀 Wie man mitliest

1. Beginne mit [Kapitel 1](chapters/01-welcome-to-rust-city.md)
2. Folge den Charakteren durch ihre Abenteuer
3. Probiere die Code-Beispiele auf [play.rust-lang.org](https://play.rust-lang.org)
4. Diskutiere im [GitHub Discussions](https://github.com/DEINUSERNAME/rust-city-chronicles/discussions)

## 🤝 Mitmachen

Fehler gefunden? Ideen für neue Kapitel?  
- [Issue erstellen](https://github.com/DEINUSERNAME/rust-city-chronicles/issues)
- [Pull Request senden](https://github.com/DEINUSERNAME/rust-city-chronicles/pulls)
- [Diskussion starten](https://github.com/DEINUSERNAME/rust-city-chronicles/discussions)

## 📬 Updates

⭐ **Star dieses Repo** um Updates zu erhalten!
